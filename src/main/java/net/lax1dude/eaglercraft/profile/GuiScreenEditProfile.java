...
import dev.colbster937.eaglercraft.gui.GuiTextField;
...
public class GuiScreenEditProfile extends GuiScreen {
	...
	public void initGui() {
		...
		this.username = new GuiTextField(this.fontRenderer, this.width / 2 - 20 + 1, this.height / 6 + 24 + 1 + trans, 138,
				20, EaglerProfile.getName());
		this.username.isEnabled = true;
		selectedSlot = EaglerProfile.presetSkinId == -1 ? EaglerProfile.customSkinId
				: (EaglerProfile.presetSkinId + EaglerProfile.skins.size());
		this.controlList.add(new GuiButton(200, this.width / 2 - 100, this.height / 6 + 168, "Done"));
		// this.controlList.add(new GuiButton(2, this.width / 2 - 21, this.height / 6 +
		// 110, 71, 20, I18n.format("profile.addSkin")));
		// this.controlList.add(new GuiButton(3, this.width / 2 - 21 + 71, this.height /
		// 6 + 110, 72, 20, I18n.format("profile.clearSkin")));
	}
	...
	public void drawScreen(int mx, int my, float par3) {
		...
		this.drawString(this.fontRenderer, "Username", this.width / 2 - 20, this.height / 6 + 8 + trans, 10526880);
		this.drawString(this.fontRenderer, "Player Skin", this.width / 2 - 20, this.height / 6 + 66 + trans, 10526880);
		...
		if (dropDownOpen) {
			...
			for (int i = 0; i < slotsVisible; i++) {
				if (i + scrollPos < dropDownOptions.length) {
					...
					this.fontRenderer.drawStringWithShadow(dropDownOptions[i + scrollPos], skinX + 5, skinY + 5 + i * 10,
							14737632);
				}
			}
		...
		playerModel.render(0.0f, 0.0f, (float) (System.currentTimeMillis() % 100000) / 50f, ((xx - mx) * 0.06f),
				((yy - my) * -0.1f), 1.0F, RubyUtils.getPlayerScale(0.0625F));
	...
	protected void drawSlotInventory(int par1, int par2, int par3) {
		super.drawSlotInventory(par1, par2, par3);
		...
		if (par3 == 0) {
			...
			if (dropDownOpen && !dragging) {
				for (int i = 0; i < slotsVisible; i++) {
					if (i + scrollPos < dropDownOptions.length) {
						if (selectedSlot != i + scrollPos) {
							if (par1 >= skinX && par1 < (skinX + skinWidth - 10) && par2 >= (skinY + i * 10 + 5)
									&& par2 < (skinY + i * 10 + 15) && selectedSlot != i + scrollPos) {
