...
public class DirStorage {
  ...
  public DirStorage(Minecraft mc, String name) {
    this.mc = mc;
    this.root = new VFile2(Minecraft.getMinecraft().mcDataDir, name);
    this.files = new VFile2(root, name + ".json");
    this.filesArr = JSONUtils.stringToArray(this.files.getAllChars());
  }

  public void _import(LoadingScreenRenderer prog, String name, byte[] data) {
    String _name = name;
    try {
      int count = 0;
      ZipEntry entry;
  ...
  public void _import(LoadingScreenRenderer prog, FileChooserResult result) {
  ...
  public void export(LoadingScreenRenderer prog, String name) {
  ...
  public void delete(LoadingScreenRenderer prog, String name) {
    VFile2 _root = (new VFile2(this.root, name));
    List<VFile2> files = _root.listFiles(true);
    int subpath = _root.getPath().length();
    int i = 0;
  ...
  public void addToManifest(LoadingScreenRenderer prog, String name) {
  ...
  public void removeFromManifest(LoadingScreenRenderer prog, String name) {
    JSONArray arr = new JSONArray();
    for (String file : getFilesInManifest())
      if (!file.equals(name))
        arr.put(file);
  ...
  public void updateProgress(LoadingScreenRenderer prog, String title, String text, int perc) {
    if (prog != null) {
      if (!ScuffedUtils.isStringEmpty(title)) prog.displayProgressMessage(title);
      if (!ScuffedUtils.isStringEmpty(text)) prog.displayLoadingString(text);
      if (perc != -2) prog.setLoadingProgress(perc);
    }
  }

  public void updateProgress(LoadingScreenRenderer prog, String title) {
    this.updateProgress(prog, title, "", -2);
  }

  public void updateProgress(LoadingScreenRenderer prog, String title, String text) {
    this.updateProgress(prog, title, text, -2);
  }

  public void updateProgress(LoadingScreenRenderer prog, String text, int perc) {
    this.updateProgress(prog, "", text, perc);
  }

  private void updateManifest(LoadingScreenRenderer prog) {
