...
public class EntityPlayer extends EntityLiving {
	...
	public final void preparePlayerToSpawn() {
		this.yOffset = RubyUtils.getPlayerScale(1.62F);
		this.setSize(0.6F, RubyUtils.getPlayerScale(1.8F));
	...
	public final void dropPlayerItemWithRandomChoice(ItemStack var1, boolean var2) {
		if (var1 != null) {
			...
			var4.motionX = -MathHelper.sin(this.rotationYaw / 180.0F * (float) Math.PI)
					* MathHelper.cos(this.rotationPitch / 180.0F * (float) Math.PI) * 0.3F;
			var4.motionZ = MathHelper.cos(this.rotationYaw / 180.0F * (float) Math.PI)
					* MathHelper.cos(this.rotationPitch / 180.0F * (float) Math.PI) * 0.3F;
	...
	protected final float getEyeHeight() {
		return RubyUtils.getPlayerScale(0.12F);
	}
